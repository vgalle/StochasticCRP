...
% We solve until both methods do not work
        while instance <= 30
            ...
            disp(strcat('EG'));
            OBJ(instance+1,6) = heuristic(initialBay,1,nSamples);
            disp(strcat('EM'));
            OBJ(instance+1,7) = heuristic(initialBay,2,nSamples);
            disp(strcat('ERI'));
            OBJ(instance+1,8) = heuristic(initialBay,3,nSamples);
            disp(strcat('L'));
            OBJ(instance+1,9) = heuristic(initialBay,4,nSamples);
            disp(strcat('Rand.'));
            OBJ(instance+1,10) = heuristic(initialBay,5,nSamples);
            disp(strcat('Optimal Algorithm'));
            [OBJ(instance+1,4),~,solvingTime] = PBFSA(initialBay,LowerBoundType,0,timeLimit);
